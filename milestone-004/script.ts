...
function generateResume(event: Event) {
    ...
    // Clear existing resume content from both the display and local storage
    resumeOutput.innerHTML = '';
    localStorage.removeItem("editableResumeContent");

    // Creating HTML for editable resume display
    const resumeHTML = `
        <h1>Editable Resume</h1><b>
...
// Save changes directly to local storage for persistence
function saveChangesToLocalStorage() {
    const resumeContent = resumeOutput.innerHTML;
    localStorage.setItem("editableResumeContent", resumeContent);
}

// Clear local storage on page reload to avoid displaying old content
window.addEventListener("beforeunload", () => {
    localStorage.removeItem("editableResumeContent");
});

// Load saved resume content on page load
function loadResumeFromLocalStorage() {
    const savedContent = localStorage.getItem("editableResumeContent");
    if (savedContent) {
        resumeOutput.innerHTML = savedContent;
    }
}

document.getElementById("generateResumeBtn")?.addEventListener("click", generateResume);
window.addEventListener("load", loadResumeFromLocalStorage);
